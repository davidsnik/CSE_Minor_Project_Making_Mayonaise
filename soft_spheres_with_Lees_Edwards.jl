...
function lees_edwards_separation(xi, xj, box_side, γ)
    ...
    # Wrap y with standard periodicity
    dy = wrap_x(dy_raw, box_side)
    
    # Wrap x accounting for shear displacement
...
function shear_stress_soft(sys::Molly.System, box_side::Float64, γ::Float64=0.0)
    ...
    @inbounds for i in 1:N-1
        xi = coords[i]
        for j in i+1:N             
            dr = lees_edwards_separation(xi, coords[j], box_side, γ)
            r = norm(dr)
            if r <= eps(Float64)
                continue
            end
...
function compute_moduli_over_history(coords_history, velocities_history, atoms, box_side, cutoff, pairwise_inter, walls; gamma_fn, save_every, dt)
    ...
    for (idx, frame) in enumerate(coords_history)
        ...
        γ, _ = shear_state(shear_profile, (idx-1) * save_every * dt)
        # compute stress for this frame
        sigma_xy = shear_stress_soft(sys_tmp, box_side, γ)
...
function apply_lees_edwards!(sys::Molly.System, box_side::Float64, t::Float64, shear_profile)
    γ, γ̇ = shear_state(shear_profile, t)
    Ly = box_side
    Δx = γ * Ly
    ...
    @inbounds for i in eachindex(sys.coords)
        ...
        # Cross top boundary
        if p[2] ≥ Ly
            sys.coords[i] = SVector(
                mod(p[1] - Δx, box_side),
                p[2] - Ly
            )
            sys.velocities[i] = SVector(
                v[1] - γ̇ * Ly,
                v[2]
            )
...
init_coord = Molly.place_atoms(n_droplets, boundary; min_dist=minimal_dist_frac * 2 * droplet_radius)
n_bulk   = n_droplets
...
# initial stress
push!(sigma_history, shear_stress_soft(sys, box_side, 0.0))
push!(sigma_time, 0.0)
#endregion
...
post_log! = (step_idx -> begin
    ...
    if step_idx % save_every == 0 || step_idx == nsteps
        ...
        γ, _ = shear_state(shear_profile, step_idx * dt)
        sigma_xy = shear_stress_soft(sys, box_side, γ)
