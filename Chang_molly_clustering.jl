...
function build_emulsion_system(x0_all::Vector{SVector{2,Float64}},
                               ...
                               box_side::Float64, cutoff::Float64, velocities, droplets_matrix)
...
#                                box_side::Float64, cutoff::Float64, velocities)
    ...
    cellListMap_finder = Molly.CellListMapNeighborFinder(
        eligible=droplets_matrix, # all bulk particles interact
        dist_cutoff=cutoff,
        x0=x0_all[1:n_bulk],
        unit_cell = boundary,
...
volume_fraction_oil = 0.9
box_side            = 32.2
cutoff              = 1.0
density_number      = 10.0    # particles per unit area
...
x0_oil, x0_water, droplets_matrix, n_per = if volume_fraction_oil <= 1
    ...
    println("Centers generated: ", centers)

    x0_oil, R_used, droplets_matrix, n_per = generate_multi_droplet_and_matrix(
        n_oil,
        n_water,
        centers,
        droplet_area,
    ...
    x0_oil, x0_water, droplets_matrix, n_per
else
    ...
    x0_water, R_used, droplets_matrix, n_per = generate_multi_droplet_and_matrix(
        n_water,
        n_oil,
        centers,
        droplet_area,
...
sys = build_emulsion_system(x0_all, n_oil, n_water, box_side, cutoff, v0_all, droplets_matrix)
...
water_colors = [RGB(0.2, 0.4, 0.8) for _ in 1:n_water] # all water particles blue
colors = vcat(oil_colors, water_colors)
...
# Build filename
fname = "emulsion_molly_positive_velocity_verlet_$(dt)_$(a_oil_water)_$(a_water_water)_$(n_droplets)_$(volume_fraction_oil).mp4"
...
visualize(
    ...
    color = colors,
    markersize = 0.8,
    framerate = Int(nsteps/(50*100)), # so the simulation will last approx 50 seconds
)
