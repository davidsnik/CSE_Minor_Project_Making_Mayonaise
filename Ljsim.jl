...
#adjust time
dt = 0.005
T = 4.0 #CHANGE TIME HERE
nsteps = Int(round(T/dt))

@inline wrap_x(dx, L) = dx - L * round(dx / L)

function no_overlap(pos::SVector{2,Float64},
    ...
    for p in positions
        dx = pos[1] - p[1]
        dx -= box_side * round(dx / box_side)   # periodic x
        dy = pos[2] - p[2]                      # y handled by walls/periodic
...
include(joinpath(@__DIR__, "functions2.jl"))
...
const LJ_SIGMA = 1.0
const MIN_DIST = 0.75 * LJ_SIGMA

@inline function lj_force_potential(ϵ::Real, r::Real, rc::Real)
    if r >= rc
        return 0.0, 0.0
    end
    ...
    # LJ force magnitude
    f = 0.0000001 *24 * ϵ / r * (2 * invr12 - invr6)

    # LJ potential
    U = 4 * ϵ * (invr12 - invr6)

    # Shifted-force correction (force → 0 at rc)
    invrc  = σ / rc
    invrc2 = invrc^2
    invrc6 = invrc2^3
    invrc12 = invrc6^2
    f_rc = 24 * ϵ / rc * (2 * invrc12 - invrc6)
    U_rc = 4 * ϵ * (invrc12 - invrc6)

    # Apply force and energy shift
    f -= f_rc * (r / rc)
    U -= U_rc + (r - rc) * f_rc
...
function Molly.force(inter::EmulsionInter,
    ...
    fmag, _ = lj_force_potential(a_ij, r, inter.cutoff) # get scalar force magnitude
    return fmag * disp / r 
end
...
function emulsion_energy(sys::Molly.System, cutoff::Float64, box_side::Float64; n_bulk_only::Union{Nothing,Int}=nothing)
    ...
    @inbounds for i in 1:N-1
        ...
        for j in i+1:N
            ...
            r = sqrt(dx*dx + dy*dy)
            _, U = lj_force_potential(a, r, cutoff)
            E_pot += U
        end
    end
...
const a_water_water = 0.1 # 5 <- better
const a_oil_oil     = 0.1 # 5 <- better
const a_oil_water   = 0.12 # 100 <- better

const a_wall_wall   = 0.1
const a_wall_water  = 0.12
const a_wall_oil    = 0.12
...
for i in 1:length(x0_emulsion)-1
    for j in i+1:length(x0_emulsion)
        dx = x0_emulsion[j][1] - x0_emulsion[i][1]
        dx -= box_side * round(dx / box_side)
        dy = x0_emulsion[j][2] - x0_emulsion[i][2]
        r2 = dx*dx + dy*dy
        if r2 < MIN_DIST^2
            error("Initial overlap detected between particles $i and $j (r = $(sqrt(r2)))")
        end
    end
end
println("No initial overlaps detected")
