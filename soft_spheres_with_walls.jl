...
function my_custom_shortcut(atom_i, atom_j)
    # Example: Ignore interaction if both particles are wall
    if atom_i.atom_type == 2 || atom_j.atom_type == 2
        return true
    end
    return false
end
...
wall_radius    = 0.1 * droplet_radius
wall_energy_strength = 10 * energy_strength
...
if droplet_radius*2 > skipping_neighbors_threshold
    pairwise_inter = (Molly.SoftSphere(use_neighbors=false, shortcut=my_custom_shortcut),)
else
    pairwise_inter = (Molly.SoftSphere(cutoff=Molly.DistanceCutoff(cutoff), use_neighbors=true, shortcut=my_custom_shortcut),)
...
if enable_walls
    ...
    wall_atoms = [Molly.Atom(mass=wall_mass, σ=wall_radius, ϵ=wall_energy_strength, atom_type=2) for _ in 1:length(walls)]
else
...
@show typeof(wall_atoms)
sys, wall_range = build_soft_emulsion_system(
    init_coord,
    walls,
    droplets,
    wall_atoms,
...
post_wall! = isempty(wall_indices) ? nothing :
   (step_idx -> begin
    t = step_idx * dt
    enforce_wall_motion!(sys, wall_indices, wall_bases, wall_sides, shear_profile, t, wall_gap, box_side)
