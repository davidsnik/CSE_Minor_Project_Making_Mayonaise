    ...
    for (k, center) in enumerate(centers)
        for i in 1:n_per[k]
            ...
            while !placed && attempts < 10_000
                ...
                if no_overlap(pos, oil, MIN_DIST, box_side)
                    push!(oil, pos)
                    placed = true
                end
            end
            ...
            if !placed
                error("Could not place oil bead without overlap in droplet $k")
            end
        end
    end
...
function generate_multi_droplet_and_matrix(n_oil::Int, n_water::Int,
    ...
    for (k, center) in enumerate(centers)
        ...
        for i in 1:n_per[k]
            ...
            if no_overlap(p, oil, MIN_DIST, box_side)
                push!(oil, p)
...
function generate_outside_droplets(n_water::Int,
    ...
    while length(water) < n_water && attempts < max_attempts
        ...
        if !inside_any_droplet &&
            !close_to_wall &&
            no_overlap(p, water, MIN_DIST, box_side)
